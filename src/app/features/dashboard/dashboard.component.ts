...
export default class DashboardComponent {
  ...
  dataSourcePendingProducts = computed(() =>
    this.pendingProducts.hasValue()
      ? this.pendingProducts.value()!.slice(0, 7)
      : []
  );
  dataSourceProducts = computed(() =>
    this.products.hasValue() ? this.products.value()!.slice(0, 7) : []
  );
  hasPendingProducts = computed(
    () =>
      this.pendingProducts.hasValue() &&
      this.pendingProducts.value()!.length > 0
  );
